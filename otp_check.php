...
if(isset($_POST['verify']))
{
    ...
    $sql = "SELECT `otp`,`name` from user where phone = $phone";
    ...
    if($result->num_rows > 0){
        while($row = $result->fetch_assoc()) {
            if($row["otp"] == "$otp")
            {
            $sql = "UPDATE user SET otp = 'verified' WHERE phone = $phone";
        ...
        echo "<h4 style='text-align:center;color:red;margin-top:50px;'>"."Error Occured Check if you are registered!"."</h4>";
    }
    $conn->close();
}
?>
   ...
   <body>
      <div class="container-fluid" style="background-color:white;">
         ...
         <form action="otp_check.php" method="POST">
            ...
            <label for="text" style="font-size:22px;">Enter OTP sent to your Phone no: <b style="color:#017AFD;"><u><?php echo $_SESSION['phone']; ?></u></b> </label>
            <br/><br/>
            <i class="animated tada fas fa-mobile-alt fa-10x"></i>
            <br/><br/>
            <input type="text" class="form-control" style="width:280px;margin:0 auto;" name="otp_check">
            </div>
         <button type="submit" class="btn btn-primary" name="verify" style="width:280px;">Verify</button><br/><br/>
         <label for="text" style="font-size:18px;"><a href="success.php?resend=true" style="text-decoration:underline;">Resend OTP</a></label>
            </form>
         </div>
        </div>
